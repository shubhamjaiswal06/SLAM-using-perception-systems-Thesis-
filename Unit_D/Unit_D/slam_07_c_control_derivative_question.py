...
class ExtendedKalmanFilter:
    ...
    def dg_dcontrol(state, control, w):
        ...
        if r != l:
            ...
            dg2_dl = (cos(theta) + (l/w)*sin(theta))/2
            dg2_dl = -1/w
            dg2_dr = (-w*l/(r-l)**2)*(sin(theta + alpha) - sin(theta)) + ((r+l)/(2*(r-l)))*cos(theta+alpha)   
            dg3_dl = (-w*l/(r-l)**2)*(-cos(theta + alpha) + cos(theta)) + ((r+l)/(2*(r-l)))*sin(theta+alpha)
            dg3_dr = 1/w
