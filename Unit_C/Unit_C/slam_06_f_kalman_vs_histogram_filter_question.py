...
def histogram_filter_step(belief, control, measurement):
    ...
    #print(correction.offset + len(correction.values)/2)
...
def kalman_filter_step(belief, control, measurement):
    ...
    #print(control.mu)
    mu = belief.mu + control.mu
    sigma2 = belief.sigma2 + control.sigma2
    prediction = Density(mu, sigma2)

    # Correction.
    K = sigma2/(sigma2 + measurement.sigma2)
    mu_ = mu + K*(measurement.mu - mu)
    sigma2_ = (1 - K)*sigma2
