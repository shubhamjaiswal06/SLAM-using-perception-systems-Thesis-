...
class ExtendedKalmanFilterSLAM:
    ...
    def g(state, control, w):
        ...
        #print(state.shape)
        m = concatenate((array([g1, g2, g3]),array(state[3:state.shape[0]])), axis = 0)
        #print(m.shape)
        return m
    ...
    def predict(self, control):
        ...
        Z3_2N = zeros((3,2*self.number_of_landmarks))
        Z_2N = zeros((2*self.number_of_landmarks,2*self.number_of_landmarks))
        I = eye(2*self.number_of_landmarks)

        temp1 = concatenate((G3, Z3_2N), axis = 1)
        temp2 = concatenate((Z3_2N.T, I), axis = 1)
        G = concatenate((temp1, temp2), axis = 0)

        tmp1 = concatenate((R3, Z3_2N), axis = 1)
        tmp2 = concatenate((Z3_2N.T, Z_2N), axis = 1)
        R = concatenate((tmp1, tmp2), axis = 0)
    ...
    def add_landmark_to_state(self, initial_coords):
        ...
        self.state = concatenate((self.state, initial_coords), axis = 0)
        #print(self.state)
        Z3_2N = zeros((3,2*self.number_of_landmarks))
        sigma_x1_2 = 10**10
        sigma_y1_2 = 10**10
        L_covariance = eye(2*self.number_of_landmarks,2*self.number_of_landmarks)
        L_covariance = L_covariance*(10**10)
        #L_covariance[0][0] =  sigma_x1_2
        #L_covariance[1][1] =  sigma_y1_2
        #print(L_covariance.shape)
        #print(self.covariance.shape)
        #print(Z3_2N.shape)
        temp1 = concatenate((self.covariance[0:3,0:3], Z3_2N), axis = 1)
        temp2 = concatenate((Z3_2N.T, L_covariance), axis = 1)
        
        self.covariance = concatenate((temp1, temp2), axis = 0)
    ...
    def correct(self, measurement, landmark_index):
        ...
        H[0:2,0:3] = H3
        H[0:2, 3 + 2*landmark_index:5 + 2*landmark_index] = H3_

        
        #H = concatenate((H3, -H3_), axis = 1)
